...
@interface ELHCountdownTask : NSOperation
...
@implementation ELHCountdownTask
...
@interface ELHButtonCountdownManager ()
...
@implementation ELHButtonCountdownManager
+ (instancetype)defaultManager {
    ...
    if ([instance class] != [ELHButtonCountdownManager class]) {
                         ...
                         finished:(void (^)(NSTimeInterval))finished
{
    ...
    if ([self countdownTaskExistWithKey:aKey task:&task]) {
        ...
        task                   = [[ELHCountdownTask alloc] init];
...
- (BOOL)countdownTaskExistWithKey:(NSString *)akey
                             task:(ELHCountdownTask *__autoreleasing  _Nullable *)task
{
    __block BOOL taskExist = NO;
    
    if ([@([UIDevice currentDevice].systemVersion.doubleValue) compare:@(8)] == NSOrderedAscending) {
        [_pool.operations enumerateObjectsUsingBlock:^(__kindof ELHCountdownTask * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
